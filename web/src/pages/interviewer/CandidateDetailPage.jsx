...
export default function CandidateDetailPage() {
  ...
  return <Box><InterviewerNav /><Paper sx={{ p: 3 }}><Typography variant="h5" sx={{ fontWeight: 700 }}>Candidate Detail</Typography>{error ? <Alert severity="error" sx={{ my: 2 }}>{error}</Alert> : null}{isLoading ? <CircularProgress /> : null}{candidate ? <Stack spacing={2} sx={{ mt: 2 }}><Card variant="outlined"><CardContent><Typography variant="h6">{applicant.full_name}</Typography><Typography color="text.secondary">{applicant.email} • {applicant.phone_number || 'No phone'}</Typography><Typography>Application status: {titleize(candidate.status)}</Typography><Typography>Applied: {formatDateTime(candidate.applied_at)}</Typography><Typography>Recruiter remark: {candidate.recruiter_remark || '—'}</Typography>{applicant.linkedin_url ? <Button href={applicant.linkedin_url} target="_blank" rel="noreferrer">LinkedIn</Button> : null}{resume.resume_file ? <Button onClick={openResume}>Open resume</Button> : null}</CardContent></Card><Grid container spacing={2}><Grid item xs={12} md={6}><Card variant="outlined"><CardContent><Typography variant="h6">Resume extraction</Typography><Typography><strong>Skills:</strong> {formatExtractedValue(candidate.extracted_skills)}</Typography><Typography><strong>Experience:</strong> {formatExtractedValue(resume.extracted_experience)}</Typography><Typography><strong>Education:</strong> {formatExtractedValue(resume.extracted_education)}</Typography></CardContent></Card></Grid><Grid item xs={12} md={6}><Card variant="outlined"><CardContent><Typography variant="h6">AI scores</Typography>{Object.entries(candidate.scores ?? {}).map(([key, value]) => <Typography key={key}>{titleize(key)}: {formatExtractedValue(value)}</Typography>)}</CardContent></Card></Grid></Grid><Card variant="outlined"><CardContent><Typography variant="h6">Personal summary</Typography><Typography whiteSpace="pre-line">{applicant.personal_summary || '—'}</Typography></CardContent></Card>{interview ? <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap"><Button component={RouterLink} to={`/interviewer/interviews/${interview.id}/invitation`} variant="contained">Send invitation</Button><Button component={RouterLink} to={`/interviewer/interviews/${interview.id}`} variant="outlined">Open interview</Button></Stack> : null}</Stack> : null}</Paper></Box>;
}
