...
export default function CandidateProfilePage() {
  ...
  return <Box><RecruiterNav /><Paper sx={{ p: 3 }}>{error ? <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert> : null}{success ? <Alert severity="success" sx={{ mb: 2 }}>{success}</Alert> : null}{isLoading ? <CircularProgress /> : null}{profile ? <Stack spacing={3}><Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" spacing={2}><Box><Typography variant="h5" sx={{ fontWeight: 700 }}>{applicationName(profile)}</Typography><Typography color="text.secondary">{applicant?.email} • {applicant?.phone_number || 'No phone'}</Typography><Chip label={titleize(profile.status)} sx={{ mt: 1 }} /></Box><Stack direction="row" spacing={1} useFlexGap flexWrap="wrap"><Button component={RouterLink} to={`/recruiter/applications/${applicationId}/assign-interview`} variant="outlined">Assign interviewer</Button><Button component={RouterLink} to={`/recruiter/applications/${applicationId}/hiring-decision`} variant="outlined">Hiring decision</Button><Button color="error" onClick={reject} variant="outlined">Reject</Button></Stack></Stack><Grid container spacing={2}><Grid item xs={12} md={6}><Card><CardContent><Typography variant="h6">Resume and AI extraction</Typography><Typography><strong>Skills:</strong> {(profile.extracted_skills ?? []).join(', ') || '—'}</Typography><Typography><strong>Experience:</strong> {formatExtractedValue(profile.resume_info?.extracted_experience)}</Typography><Typography><strong>Education:</strong> {formatExtractedValue(profile.resume_info?.extracted_education)}</Typography>{profile.resume_info?.resume_file ? <Button onClick={openResume}>Open resume</Button> : null}</CardContent></Card></Grid><Grid item xs={12} md={6}><Card><CardContent><Typography variant="h6">Scores</Typography>{['semantic_score','skill_score','experience_score','education_score','final_score'].map((key) => <Typography key={key}>{titleize(key)}: {scoreText(profile.scores?.[key])}</Typography>)}</CardContent></Card></Grid></Grid><TextField label="Recruiter remark" multiline minRows={3} value={remark} onChange={(e) => setRemark(e.target.value)} /><Button onClick={saveRemark} variant="outlined">Save remark</Button><Box><Typography variant="h6">Stage history</Typography><List>{history.map((item) => <ListItem key={item.id}><ListItemText primary={`${titleize(item.from_stage)} → ${titleize(item.to_stage)}`} secondary={`${item.note || 'No note'} • ${item.changed_by_name || 'System'} • ${formatDateTime(item.changed_at)}`} /></ListItem>)}</List></Box></Stack> : null}</Paper></Box>;
}
