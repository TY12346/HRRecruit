  ...
  return <Stack spacing={2}>
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" spacing={2}>
        ...
        <Stack spacing={0.5}>
          ...
          <Stack direction="row" gap={1} flexWrap="wrap"><Chip color={live ? 'success' : 'warning'} label={live ? 'Live analysis' : data.provenance === 'seeded_demo_fixture' ? 'Seeded demo result' : 'Legacy result'} />
            <Chip label={`${data.evidencePairs.length} evidence pairs`} /><Chip label={`${data.differentWordingCount} different-wording matches`} /></Stack>
        </Stack>
      </Stack>
    </CardContent></Card>
    ...
    {!compact ? <>
      <Divider /><Box><Typography variant="h6">Sentence-BERT semantic evidence</Typography>
        ...
        {data.evidencePairs.length ? <Stack spacing={1.5}>{data.evidencePairs.map((pair, index) => <Card variant="outlined" key={`${pair.requirement_text}-${pair.rank}-${index}`}><CardContent>
          <Stack direction="row" gap={1} flexWrap="wrap"><Chip size="small" label={pair.requirement_type || 'other'} /><Chip size="small" variant="outlined" label={pair.requirement_source || 'unknown source'} /><Chip size="small" color="primary" label={pct(pair.similarity_score)} />
            {pair.different_wording ? <Chip size="small" color="secondary" label="Related meaning detected despite different wording" /> : null}</Stack>
          ...
          {pair.shared_terms?.length ? <Typography variant="body2" sx={{ mt: 1 }}>Shared meaningful terms: {pair.shared_terms.join(', ')}</Typography> : null}
