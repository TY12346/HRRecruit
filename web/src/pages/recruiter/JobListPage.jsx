...
export default function JobListPage() {
  ...
  return <Box><RecruiterNav /><Paper sx={{ p: 3 }}><Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" spacing={2} sx={{ mb: 2 }}><Box><Typography variant="h5" sx={{ fontWeight: 700 }}>Jobs</Typography></Box><Button component={RouterLink} to="/recruiter/jobs/create" variant="contained">Create requisition</Button></Stack>{error ? <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert> : null}{success ? <Alert severity="success" sx={{ mb: 2 }}>{success}</Alert> : null}{isLoading ? <CircularProgress /> : null}<Table><TableHead><TableRow><TableCell>Title</TableCell><TableCell>Status</TableCell><TableCell>Type</TableCell><TableCell>Location</TableCell><TableCell>Salary</TableCell><TableCell>Created</TableCell><TableCell align="center">Actions</TableCell></TableRow></TableHead><TableBody>{jobs.map((job) => <TableRow key={job.id}><TableCell>{job.title}</TableCell><TableCell><Chip label={titleize(job.status)} size="small" color={job.status === 'open' ? 'success' : 'default'} /></TableCell><TableCell>{job.employment_type}</TableCell><TableCell>{job.location}</TableCell><TableCell>{job.salary_range?.trim() || job.approximate_salary || 'Not specified'}</TableCell><TableCell>{formatDateTime(job.created_at)}</TableCell><TableCell align="center"><Stack direction="row" spacing={1} justifyContent="center"><Button component={RouterLink} to={`/recruiter/jobs/${job.id}`} size="small">View</Button><Button component={RouterLink} to={`/recruiter/jobs/${job.id}/edit`} size="small">Edit</Button><Button color="error" onClick={() => handleDelete(job)} size="small">Delete</Button></Stack></TableCell></TableRow>)}{!isLoading && jobs.length === 0 ? <TableRow><TableCell colSpan={7}>No jobs yet.</TableCell></TableRow> : null}</TableBody></Table></Paper></Box>;
}
