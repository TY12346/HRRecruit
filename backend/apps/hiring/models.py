...
class JobHiringDecision(models.Model):
    ...
    job_posting = models.ForeignKey(JobPosting, on_delete=models.CASCADE, related_name='hiring_decisions')
    recruiter = models.ForeignKey(User, on_delete=models.PROTECT, related_name='submitted_job_hiring_decisions', limit_choices_to={'role': User.Role.RECRUITER})
    decision_type = models.CharField(max_length=30, choices=DecisionType.choices)
    justification = models.TextField()
    status = models.CharField(max_length=30, choices=Status.choices, default=Status.DRAFT)
    submitted_at = models.DateTimeField(blank=True, null=True)
    reviewed_by = models.ForeignKey(User, on_delete=models.SET_NULL, related_name='reviewed_job_hiring_decisions', blank=True, null=True, limit_choices_to={'role': User.Role.HR_HEAD})
    reviewed_at = models.DateTimeField(blank=True, null=True)
