...
| Applicant ranking order | `test_recruiter_views_ranked_applicants_for_own_job_only` and `test_ranked_applicants_use_earliest_application_as_equal_score_tie_breaker_and_nulls_last` |
...
- **Endpoints involved**
  - `POST /api/applications/<application_id>/screen/`
  - `GET /api/jobs/<job_id>/ranked-applicants/`
  - `GET /api/applications/<application_id>/applicant-profile/`
- **Tests added/confirmed**
...
### 6. Hybrid applicant scoring and ranking
...
- **Tests added/confirmed**
  ...
  - Applicant ranking order is covered, including null scores last and earliest application as equal-score tie breaker.
...
- **Current limitations**
  ...
  - Applicant ranking is score-first and does not include fairness/bias audit metrics.
- **Fallback behavior**
...
- **Manual test steps**
  ...
  3. Call `GET /api/jobs/<job_id>/ranked-applicants/`.
  4. Verify descending `final_score`, equal-score tie by earlier `applied_at`, and unscored applicants last.
